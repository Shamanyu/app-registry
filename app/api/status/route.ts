...
// Patterns that indicate an error page (in title or body)
...
// Patterns for "placeholder / not yet deployed" pages (200 OK but app isn't live)
// e.g. PythonAnywhere "Coming Soon", generic hosting defaults
...
function looksLikeNonLivePage(html: string): boolean {
  ...
  // Very short responses often indicate error pages
...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    if (status >= 200 && status < 300) {
      ...
      live = true; // Redirect = server responding
    }
    // 4xx/5xx: live stays false
  } catch {
    live = false;
  }

  cache.set(key, { live, timestamp: Date.now() });
