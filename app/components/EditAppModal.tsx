  ...
  return (
    <dialog
      ...
      className="w-[calc(100%-2rem)] max-w-md max-h-[calc(100dvh-2rem)] overflow-y-auto
        m-auto p-0 rounded-2xl border border-violet-500/20 bg-[#0c0c10] shadow-2xl shadow-violet-500/10
        [&::backdrop]:bg-black/75 [&::backdrop]:backdrop-blur-md"
    >
      <div className="p-4 sm:p-5">
        <div className="flex items-center justify-between mb-4">
          <div>
            <h2 className="text-base font-semibold bg-gradient-to-r from-violet-200 to-cyan-200 bg-clip-text text-transparent">
              Edit App
            </h2>
            <p className="text-xs text-violet-300/50 mt-0.5">Update {project.name}</p>
          </div>
          <button
            type="button"
            onClick={onClose}
        ...
        <form onSubmit={handleSubmit} className="flex flex-col gap-3">
          <div>
            <label htmlFor="edit-name" className="block text-xs font-medium text-cyan-300/70 mb-1">App Name *</label>
            <input ref={inputRef} id="edit-name" name="name" type="text" value={form.name} onChange={handleChange}
              ...
              className="w-full rounded-lg px-3 py-2.5 text-sm bg-white/[0.05] border border-white/10 text-white placeholder:text-white/20 focus:outline-none focus:ring-2 focus:ring-violet-500 focus:border-violet-500/50 disabled:opacity-50 transition-all" />
          </div>
          <div>
            <label htmlFor="edit-url" className="block text-xs font-medium text-cyan-300/70 mb-1">URL *</label>
          ...
          <div>
            <label htmlFor="edit-description" className="block text-xs font-medium text-cyan-300/70 mb-1">Description *</label>
            <textarea id="edit-description" name="description" value={form.description} onChange={handleChange}
              ...
              className="w-full rounded-lg px-3 py-2.5 text-sm bg-white/[0.05] border border-white/10 text-white placeholder:text-white/20 resize-none focus:outline-none focus:ring-2 focus:ring-violet-500 focus:border-violet-500/50 disabled:opacity-50 transition-all" />
          </div>
          <div>
            <label htmlFor="edit-icon_url" className="block text-xs font-medium text-cyan-300/70 mb-1">Icon URL <span className="text-white/30 font-normal">(optional)</span></label>
          ...
          <div>
            <label htmlFor="edit-owner" className="block text-xs font-medium text-cyan-300/70 mb-1">Owner <span className="text-white/30 font-normal">(optional)</span></label>
          ...
          <div className="flex gap-2 pt-1">
            ...
            <button type="submit" disabled={isPending || success}
              className="flex-1 flex items-center justify-center gap-1.5 rounded-lg px-3 py-2.5 text-sm font-semibold
                bg-gradient-to-r from-violet-500 to-cyan-500 hover:from-violet-400 hover:to-cyan-400
                text-white transition-all duration-200 shadow-lg shadow-violet-500/25 disabled:opacity-60">
